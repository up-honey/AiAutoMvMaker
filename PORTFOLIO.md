...
- [x] 프로젝트 제목, 주제·스타일 통합 영상 프롬프트와 화면 비율 입력
- [x] 이미지·동영상 최대 300개 다중 선택과 선택 순서 기반 타임라인 UI
- [x] 흰색·하늘색 카드형 스튜디오와 모바일 단일 흐름·하단 CTA 반응형 UI
- [x] 영상 상세 설정 상단 팝오버의 분위기·조건부 사진 속도·AI 음악 요청·직접 음악 업로드 UI
- [x] 상단 팝오버형 출력 설정과 지원 범위 기반 720P·자동 길이·화면 비율·단일 출력 UI
- [x] 상단 소개 배너를 제거한 스크롤형 데스크톱 셸과 프리셋·직접 입력 분위기 셀렉트
- [x] 단일 열 중간 밀도 설계 폼과 영상 제작 중심 헤더
- [x] 선택 순서 기반 타임라인과 이미지가 있을 때만 표시되는 사진 초당 0.5~5장 노출 속도 조절
...
- 입력 단순화: 공통 스타일과 별도 장면 프롬프트 입력을 제거하고 영상 프롬프트 하나를 생성 장면으로 사용하며, 제목·프롬프트에 예시 placeholder를 추가했습니다.
- 영상 상세 설정: 영상 분위기, 이미지 선택 시에만 표시되는 사진 전환 속도, 배경음악 선택을 하나의 상단 팝오버에 모았습니다. AI 음악을 선택하면 분위기·악기·템포 요청 입력칸이 나타나고 직접 업로드를 선택하면 음악 파일 영역이 나타납니다.
- 출력 설정: 해상도·길이·화면 비율·출력 개수를 버튼 위쪽 팝오버에서 설명합니다. 현재 렌더 파이프라인이 지원하는 720P·자동 길이·단일 출력만 활성화하고 1080P·다중 출력은 준비 중으로 명확히 표시했습니다.
- 화면 밀도: 큰 소개 배너를 제거해 헤더 다음에 제작 폼이 바로 시작되도록 했습니다. 데스크톱에서는 앱을 `100vh`에 고정하고 바깥 세로 스크롤을 숨긴 뒤, 설계·모니터 카드가 헤더 아래 남은 높이를 동일하게 채우도록 했습니다. 설계 카드의 입력·프롬프트·설정·CTA 높이를 확장해 내부 여백을 채우고, 모니터의 빈 상태는 제목 아래 가용 영역을 기준으로 세로 중앙 정렬합니다. 더 작은 모바일 화면에서는 자연스러운 페이지 세로 흐름을 유지합니다.
- 분위기 입력: 네 개의 카드형 프리셋을 하나의 셀렉트로 줄이고 `직접 입력`을 추가해 영상 상세 설정에 배치했습니다. 직접 입력한 문구는 AI 프롬프트에 포함하고 안전한 로컬 합성 단계에는 클린 프리셋을 적용합니다.
- 가독성 조정: 헤더를 `AI 영상 만들기`와 서비스 설명 중심으로 바꾸고 설계 폼을 다시 단일 열로 배치했습니다. 가장 큰 설계 제목은 27px로 유지하고 필드 제목은 15px, 입력 내용은 14px로 높였으며 분위기 셀렉트 화살표를 오른쪽에서 18px 안쪽에 배치했습니다.
- 영향: 다중 자산·배경음악·분위기 프리셋·렌더링 계약은 유지하고, 영상 프롬프트를 단일 생성 장면으로 자동 변환해 입력 계층을 단순화했습니다.
- 검증: TypeScript 검사와 Vite 프런트엔드 프로덕션 빌드를 통과했습니다. 브라우저에서 장면 프롬프트 제거, 설정 패널의 상단 방향 표시, 이미지 유무에 따른 사진 속도 show/hide, AI 음악 요청 입력과 15px 필드 제목·14px 입력 내용을 확인했습니다. 1280×720 뷰포트에서 문서 높이와 화면 높이가 모두 720px이고, 두 카드가 638px의 남은 공간을 동일하게 채웁니다. 설계 CTA와 카드 하단 간격은 15px이며, 모니터 빈 상태의 중심 오차는 6px 이내이고 상단 팝오버도 화면 안에 표시됩니다.
